...
;   Yes    → Fresh install  (old version removed silently first)
;   No     → Upgrade in place  (files overwritten, data kept)
...
Choose how to proceed:$\n$\n\
  Yes    $\t Fresh install$\t (removes old version — recommended)$\n\
  No     $\t Upgrade in place$\t (keeps your playlists and settings)$\n\
  Cancel $\t Exit installer" \
    ...
    ; ── NO: upgrade in place — do nothing, let installer overwrite ────────────
    sg_upgrade:
    Return
